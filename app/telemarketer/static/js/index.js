...
$(document).ready(function() {
	...
	/* ----------------------- */
	// scroll to start call on mobile
	/* ----------------------- */
	$("#mobile-start-call").click(function() {
		$("html,body").animate({
	...
	/* ----------------------- */
	// replace logo if mobile
	/* ----------------------- */
	if (window.mobilecheck()) {
		$("#logo-icon")
			.find("img")
			.attr("src", "/assets/images/mobile-logo.png")
			.css("width", "60vw");
	} else {
		$("#logo-icon")
			.find("img")
	...
	/* ----------------------- */
	// auto update phone number to correct format on input
	/* ----------------------- */
    $("#phone").bind("input", function(e) {
		formatted = new libphonenumber.AsYouType('US').input($(this).val())
        $(this).val(formatted);
    });
    ...
    $("#call-form").submit(function(e){
        ...
        $.post(post_url, form_data, function(res) {
            $("#call-form-results").html(res.message);
        });
    });
	...
	function updateTrackStyle() {
		$(".track-option").each(function() {
			if ($(this).attr("checked")) {
				...
				$(this).find("span").find(".playpause").addClass("selected");
			} else {
				$(this).removeClass("selected");
				$(this).find("span").find(".playpause").removeClass("selected");
			}
		});
	}

	function uncheckAllTracks() {
		$(".track-option").each(function() {
	...
	/* ----------------------- */
    $(".audio-controller").each(function() {
